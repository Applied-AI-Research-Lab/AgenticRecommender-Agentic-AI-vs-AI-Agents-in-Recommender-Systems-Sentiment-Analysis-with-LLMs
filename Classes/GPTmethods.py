...
class GPTmethods:
    def __init__(self, params):
        ...
        """
        # Access the OpenAI API key from config file (loaded from .env)
    ...
    def gpt_prediction(self, input):
        conversation = [
            ...
            {'role': 'user', 'content': self.generate_prompt(feature=input['title'] + '\n' + input[self.feature_col])}
        ]

        while True:
            conversation_response = self.gpt_conversation(conversation)  # Call the model
            ...
            cleaned_response = self.clean_response(response=content, a_field=input[self.feature_col])
...
"""
params = {
    'model_id': 'gpt-5-2025-08-07',  # Specifies the GPT model ID for making predictions.
    'prediction_column': 'gpt-5_prediction',  # Specifies the column where predictions will be stored.
    ...
    'system': 'You are an expert sentiment analysis system for product reviews.',
    ...
    # You can pass anything within brackets [example], which will be replaced during generate_prompt().
     'prompt': '''You are an expert sentiment analysis system for product reviews. Your task is to predict the star rating (1-5) that a user would give based on their review text.
...
Review:''',
    'feature_col': 'text',  # Specifies the column in the dataset containing the text input/feature for predictions.
    'label_col': 'rating',  # Used only for creating training and validation prompt-completion pairs JSONL files.
    'json_key': 'rating',  # Defines the key in the JSON response expected from the model, e.g. {"category": "value"}
...
# params['model_id'] = 'gpt-5-2025-08-07' # reasoning capabilities derived from o3
# params['prediction_column'] = 'gpt-5_prediction'

# params['model_id'] = 'gpt-5-mini-2025-08-07'  # gpt-5-mini is the successor to o4-mini
# params['prediction_column'] = 'gpt-5-mini_prediction'

# params['model_id'] = 'gpt-5-nano-2025-08-07'
# params['prediction_column'] = 'gpt-5-nano-2025-08-07_prediction'

# params['model_id'] = 'gpt-4.1-2025-04-14'
# params['prediction_column'] = 'gpt-4.1_prediction'


# === GPT Model Family ===
# GPT-5:
#   • Flagship model for coding, reasoning, and agentic tasks across domains.
#   • Excels in complex, multi-domain reasoning and autonomous workflows.
#
# GPT-5 Mini:
#   • Faster, more cost-efficient version of GPT-5.
#   • Ideal for well-defined tasks and precise, structured prompts.
#
# GPT-5 Nano:
#   • Fastest and most cost-effective GPT-5 variant.
#   • Great for summarization, classification, and lightweight tasks.
#
# GPT-4.1:
#   • Smartest non-reasoning model with strong instruction-following and tool use.
#   • Offers a 1M-token context window and low latency (no reasoning step).
#
# --- Pricing ---
